...
- `indicia login` device-code sign-in and `indicia login --browser` OAuth
  authorization-code sign-in. Tokens are stored in
  `~/.config/indicia/credentials.json`.
- `indicia logout` removes saved credentials.
- `INDICIA_AUTH_URL` and `INDICIA_CONFIG_DIR` environment variables.

### Changed

- Authenticated requests send `Authorization: Bearer` when using a saved
  OAuth token. `INDICIA_API_KEY` still wins and is sent as `x-api-key`.
...
- `--enhanced` flag for the `person`, `address`, `phone`, and `email` searches: adds more results and associated people (+2 credits per lookup).

## [0.5.0] - 2026-08-02

### Added

- `link-resolver` search: resolve a TikTok or Instagram share link to identify the user who shared it (and the post author for Instagram).
